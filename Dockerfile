...
RUN apt-get update -y && apt-get install curl bash -y  \
    && curl -L https://github.com/argoproj/argo-rollouts/releases/download/v1.5.0/kubectl-argo-rollouts-linux-amd64 -o /usr/local/bin/kubectl-argo-rollouts \
    ...
    && curl -sSL https://github.com/argoproj/argo-cd/releases/download/v2.7.1/argocd-linux-amd64 -o /usr/local/bin/argocd \
